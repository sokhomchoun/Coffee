<template>
    ...
    <div id="sidebar" class="drawer max-w-[48rem]">
        ...
        <div class="drawer-body">
            <form @submit.prevent="save">
                <div class="form-row">
                    ...
                    <div class="form-col-12">
                        <div class="form-col-12 sm:form-col-6">
                            <label for="product_type" class="db-field-title required">{{ $t("label.product_type") }}</label>
                            <vue-select class="db-field-control f-b-custom-select" id="product_type"
                                v-bind:class="errors.product_type ? 'invalid' : ''"
                                v-model="productType" :options="productTypes" label-by="name"
                                @click="selectProductType"
                                value-by="id" :closeOnSelect="true" :searchable="true" :clearOnClose="true" placeholder="--"
                                search-placeholder="--" />
                            <small class="db-field-alert" v-if="errors.product_type">{{ errors.product_type[0] }}</small>
                        </div>
                    </div>
                    <div class="form-col-12">
                        <div v-if="props.form.product_type === 1" class="block sm:flex">
                            <div class="form-col-12 sm:form-col-6">
                                <label for="price" class="db-field-title required">{{ $t("label.price") }}</label>
                                <input v-on:keypress="floatNumber($event)" v-model="props.form.price"
                            ...
                            <div class="form-col-12 sm:form-col-6">
                                <label for="stock_alert" class="db-field-title required">{{ $t("label.stock_alert") }}</label>
                                <input v-on:keypress="floatNumber($event)" v-model="props.form.stock_alert"
                                    v-bind:class="errors.stock_alert ? 'invalid' : ''" type="text" id="stock_alert"
                                    class="db-field-control">
                                <small class="db-field-alert" v-if="errors.stock_alert">{{ errors.stock_alert[0] }}</small>
                            </div>
                        </div>
                        <div v-if="props.form.product_type === 2" class="block sm:flex">
                            <div class="form-col-12 sm:form-col-4">
                                <label for="variation" class="db-field-title required">{{ $t("label.variation") }}</label>
                                <vue-select class="db-field-control f-b-custom-select" id="variation"
                                    v-bind:class="errors.variation ? 'invalid' : ''"
                                    v-model="props.form.variation" :options="itemCategories" label-by="name"
                                    value-by="id" :closeOnSelect="true" :searchable="true" :clearOnClose="true" placeholder="--"
                                    search-placeholder="--" />
                                <small class="db-field-alert" v-if="errors.variation">{{ errors.variation[0] }}</small>
                            </div>

                            <div class="form-col-12 sm:form-col-4">
                                <label for="variation_type" class="db-field-title required">{{ $t("label.variation_type") }}</label>
                                <vue-multiselect id="variation_type"
                                    v-bind:class="errors.variation_type ? 'invalid' : ''" placeholder="Select Option"
                                    v-model="props.form.variation_type" :options="itemVariation" :multiple="true"
                                    :close-on-select="false" :clear-on-select="false" label="name" track-by="id" />
                                <small class="db-field-alert" v-if="errors.variation_type">{{ errors.variation_type[0] }}</small>
                            </div>
...
export default {
    ...
    data() {
        return {
            ...
            productType: this.props.form.product_type,
            image: "",
            errors: {},
        }
    },
    computed: {
        ...
        itemVariation: function () {
            const variations = this.$store.getters['variation/lists'];
            if (variations.length > 0 && variations[0].type) {
                return variations[0].type;
            }
            return [];
    ...
    mounted() {
        ...
        this.$store.dispatch('variation/lists', {
            order_column: 'id',
            order_type: 'asc',
            status: statusEnum.ACTIVE
        });
    ...
    methods: {
        selectProductType: function () {
            ...
            this.props.form.product_type = this.productType;
        },
        floatNumber(e) {
        ...
        save: function () {
            try {
                const variationTypeClean = this.props.form.variation_type.map(item => ({
                    id: item.id,
                    name: item.name
                }));
                ...
                fd.append('product_type', this.props.form.product_type);
                fd.append('variation', this.props.form.variation);
                fd.append('variation_type', JSON.stringify(variationTypeClean));
                fd.append('stock_alert', this.props.form.stock_alert);
