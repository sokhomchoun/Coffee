<template>
    ...
    <div id="sidebar" class="drawer max-w-[30rem]">
        <div class="drawer-header">
            <h3 class="drawer-title">{{ $t("menu.brand") }}</h3>
...
export default {
    ...
    computed: {
        addButton: function () {
            return { title: this.$t("button.add_brand") }
        },
    },
    watch: {
        'props.form': {
    ...
    methods: {
        reset: function() {
            ...
            this.$store.dispatch('brand/reset').then().catch();
            this.errors = {};
        ...
        save: function() {
            try {
                const tempId = this.$store.getters['brand/temp'].temp_id;
                this.loading.isActive = true;
                this.$store.dispatch('brand/save', {
                    form: this.fd,
                    search: this.props.search
                }).then((res) => {
                    appService.sideDrawerHide();
                    this.loading.isActive = false;
                    alertService.successFlip((tempId === null ? 0 : 1), this.$t('menu.brand'));
                    this.errors = {};
                    this.fd = {};
                }).catch((err) => {
                    this.loading.isActive = false;
